...
//************************************************************************************************************	
	...
	public function buscar_like($buscar,$id_grupo,$limite="")
	{
		...
		$this->db = new db;
		$lista = $this->db->query("SELECT * FROM vpersonas WHERE (identificacion LIKE '%$buscar%' OR nombre LIKE '%$buscar%' OR apellido LIKE '%$buscar%') AND id_grupo>='$id_grupo'$limite");
		if(!$lista->num_rows)
		{
			$this->mensaje = "No se encontraron registros..."."SELECT * FROM vpersonas WHERE identificacion LIKE '%$buscar%' OR nombre LIKE '%$buscar%' OR apellido LIKE '%$buscar%'$limite";
			$this->msgTipo = "aviso";
			$this->estatus = false;
			$this->json = json_encode($this);
			return $this->estatus;
		...
		{
			$item['fecha_nacimiento']= fecha($item['fecha_nacimiento']);
			$this->datos[]=$item;
		}
